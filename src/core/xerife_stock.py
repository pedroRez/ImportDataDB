...
class XerifeValidationResult:
    ...
    def preview_text(self, max_rows: int = 12) -> str:
        lines = [
            ...
            f"Linhas ignoradas: {self.skipped_rows}",
            f"Erros: {len(self.issues)}",
        ]
...
class XerifeStockImporter:
    ...
    def _prepare_row(self, row: pd.Series, row_number: int) -> Dict[str, Any] | ValidationIssue | None:
        ...
        unit_resolution = self._resolve_unit(raw_unit, nome)
    ...
    def _parse_unit_rules(self, unit_rules: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for rule in unit_rules:
            ...
            if mode == "direct":
                ...
                config["package_units"].update(
                    self._normalize_unit(value) for value in rule.get("units", [])
                )
                config["package_pattern"] = re.compile(str(rule.get("pattern", "")), re.IGNORECASE)
                config["package_aliases"].update(
                    {
                        self._normalize_unit(source): self._normalize_unit(target)
    ...
    def _resolve_unit(self, raw_unit: str, nome: str) -> Dict[str, Any] | str:
        ...
        if normalized_unit in self._unit_rules["package_units"]:
            pattern = self._unit_rules["package_pattern"]
            match = pattern.search(nome) if pattern else None
            if not match:
                return (
                    f"Sigla '{normalized_unit}' exige conteudo parseavel em NOME FANTASIA, "
                    "mas o padrao nao foi encontrado."
                )
            raw_content_unit = self._normalize_unit(match.group("unit"))
            content_unit = self._unit_rules["package_aliases"].get(raw_content_unit, raw_content_unit)
